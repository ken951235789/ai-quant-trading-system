...
def map_continuous_action(
    ...
    *,
    ...
    if config.normalized_action_space:
        normalized = float(
            np.clip(action_value, -1.0 if config.allow_short else 0.0, 1.0)
        )
        directional_target = (
        ...
        normalized = float(
            np.clip(action_value, -short_limit, config.max_position_fraction)
        )
    ...
    if magnitude <= config.hold_action_threshold:
        target = float(
            np.clip(current_position, -short_limit, config.max_position_fraction)
        )
